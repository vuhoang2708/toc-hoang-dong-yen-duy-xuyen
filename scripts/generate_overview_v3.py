...
segments = [
    ...
    },
    {
        ...
        "page": "genealogy-tree/index.html",
        "selector": "body",
        "voice": FPT_VOICE
    },
    {
...
async def take_screenshots():
    ...
    async with async_playwright() as p:
        ...
        for seg in segments:
            ...
            # Chế độ thông minh: QR code dùng file cục bộ, các trang khác dùng Vercel để lấy CSS chuẩn nhất
            if seg['page'] == "qr_page.html":
                page_url = f"file:///{os.path.abspath(seg['page'])}".replace("\\", "/")
            else:
                page_url = f"{LIVE_URL}/{seg['page']}" if seg['page'] != "index.html" else LIVE_URL
                
            print(f"[{seg['id']}] Accessing {page_url}...")
            
            try:
                await page.goto(page_url, wait_until="load", timeout=60000)
            except Exception as e:
                print(f"Warning: navigation timeout on {page_url}. Proceeding anyway.")
            ...
            try:
                if seg['selector'] and await page.locator(seg['selector']).count() > 0:
                    ...
                    # Nếu là QR page, chờ lâu hơn chút để ảnh từ API load về
                    if "qr_page" in seg['page']:
                        await asyncio.sleep(4)
                    else:
                        await asyncio.sleep(1) # Chờ cuộn trang ổn định
